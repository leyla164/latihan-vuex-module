<template>
    <div>
        ...
        <form @submit.prevent="createUser">
           ...
           <label>Steet :</label>
           <input v-model="newUser.address.steet" type="text" required ><br>
           <label>Number :</label>
           ...
           <button type="submit"
               class="grid cursor-pointer select-none rounded-md border-indigo-500 bg-indigo-500 py-2 px-5 text-center align-middle 
        ...
        <!-- <p v-if="nottification" :class="notification.type">{{ notification.message }} </p> -->
    </div>
</template>
<script>
   ...
   export default {
     ...
     computed: {
        ...mapState("notification", ["nottification"]),
     },
     methods: {
